...
// ListEnabled returns all enabled download clients ordered by priority.
// CheckDownloads iterates the full list so every client is polled, not just
// the highest-priority one (Bug #1).
